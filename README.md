...
> Note: there are other environment variables that don't need to change. Those are in `docker-compose.prod.yml` itself.
...
> Alternatively, if I have bothered to upload the latest images, you may be able to pull them with `docker-compose -f docker-compose.prod.yml pull`
...
The bot's code _is cross-platform compatible_, if you wish to develop it in a non-Linux environment.
...
The Vite server has HMR (hot module reload) enabled, so the served webpage _should reflect any
changes as soon as you save the source file_! Note that if you make changes to the proto sources,
